...
test("loading Buddhabrot uses the precomputed rotating Gaussian cloud", () => {
  ...
  assert.match(source, /mandelbrot-skipping:tuning:v9/);
  assert.match(source, /const MAX_SOURCE_DOTS = 256/);
  assert.match(source, /const MAX_PREVIEW_ITERATIONS = 100/);
  assert.match(source, /sourceDots: 64,/);
  assert.match(source, /previewIterations: 100,/);
...
test("opening and flashlight hide orbit lines, go grayscale, and throw overlapping intro rocks", () => {
  ...
  assert.match(source, /mix\(rarityTinted, gray, style\.pulse\)/);
...
test("opening waits for a Play tap and gameplay rethrow sits on the throw stone", () => {
  ...
  assert.doesNotMatch(intro, /introNameEntry|introNameInput/);
  assert.doesNotMatch(intro, /playerName|onPlayerNameChange/);
  assert.match(game, /currentIsHighscore && \(/);
  assert.match(game, /className="highscoreNameEntry"/);
  assert.match(game, /className="highscoreNameInput"/);
  const nameEntryRule = css.match(/(?:^|\n)\.highscoreNameEntry \{([^}]+)\}/)?.[1] ?? "";
  assert.match(nameEntryRule, /pointer-events:\s*auto/);
...
test("intro hides debug controls and keeps the fixed classic cloud", () => {
  ...
  assert.match(css, /\.introPaperWikiBox/);
